...
//===----------------------------------------------------------------------===//
//
// This is the entry point to the latino driver; it is a thin wrapper
// for functionality in the Driver latino library.
//
//===----------------------------------------------------------------------===//
...
#include "latino/Frontend/ChainedDiagnosticConsumer.h"
#include "latino/Frontend/CompilerInvocation.h"
#include "latino/Frontend/SerializedDiagnosticPrinter.h"
...
static void FixupDiagPrefixExeName(TextDiagnosticPrinter *DiagClient,
  ...
  // If the latino binary happens to be named cl.exe for compatibility reasons,
  // use latino-cl.exe as the prefix to avoid confusion between latino and MSVC.
  StringRef ExeBasename(llvm::sys::path::stem(Path));
  if (ExeBasename.equals_lower("cl"))
    ExeBasename = "latino-cl";
...
static int ExecuteCC1Tool(SmallVectorImpl<const char *> &ArgV) {
  // If we call the cc1 tool from the latinoDriver library (through
...
int main(int argc_, const char **argv_) {
  ...
  // are two ways to put latino in CL compatibility mode: argv[0] is either
  // latino-cl or cl, or --driver-mode=cl is on the command line. The normal
  // command line parsing can't happen until after response file parsing, so we
  // have to manually search for a --driver-mode=cl argument the hard way.
  // Finally, our -cc1 tools don't care which tokenization mode we use because
  // response files written by latino will tokenize the same way in either mode.
  ...
  // should spawn a new latino subprocess (old behavior).
  ...
  if (!DiagOpts->DiagnosticSerializationFile.empty()) {
    auto SerializedConsumer =
        latino::serialized_diags::create(DiagOpts->DiagnosticSerializationFile,
