...
//===----------------------------------------------------------------------===//
//
// This is the entry point to the latino -cc1gen-reproducer functionality, which
// generates reproducers for invocations for latino-based tools.
//
//===----------------------------------------------------------------------===//
...
struct ClangInvocationInfo {
  ...
  std::string LiblatinoOperation;
  std::string LiblatinoOptions;
...
template <> struct MappingTraits<ClangInvocationInfo> {
  static void mapping(IO &IO, ClangInvocationInfo &Info) {
    ...
    IO.mapOptional("liblatino.operation", Info.LiblatinoOperation);
    IO.mapOptional("liblatino.opts", Info.LiblatinoOptions);
...
static std::string generateReproducerMetaInfo(const ClangInvocationInfo &Info) {
  ...
  EmitStringKey("liblatino.operation", Info.LiblatinoOperation);
  EmitStringKey("liblatino.opts", Info.LiblatinoOptions);
