...
TEST_P(ImportFriendFunctions, LookupWithProtoAfter) {
  ...
  TranslationUnitDecl *FromTU =
      getTuDecl("struct X { friend void f(); };"
                ...
                // `latino::Sema::CppLookupName()` and in `LookupDirect()`)
...
TEST_P(ImportSourceLocations, OverwrittenFileBuffer) {
  ...
  SourceLocation ImportedLoc;
  {
    ...
    latino::FileManager &FM = FromSM.getFileManager();
    const latino::FileEntry &FE =
...
struct SourceWithCompletedTagList : latino::ExternalASTSource {
  std::vector<latino::TagDecl *> &CompletedTags;
  SourceWithCompletedTagList(std::vector<latino::TagDecl *> &CompletedTags)
...
TEST_P(ImportWithExternalSource, CompleteRecordBeforeImporting) {
  ...
  latino::ASTContext &Context = FromTU->getASTContext();
