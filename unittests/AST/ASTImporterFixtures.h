...
#ifndef LLVM_LATINO_UNITTESTS_AST_IMPORTER_FIXTURES_H
#define LLVM_LATINO_UNITTESTS_AST_IMPORTER_FIXTURES_H
...
::testing::AssertionResult isImportError(llvm::Expected<T> &ValOrErr,
  ...
  if (ValOrErr) {
    ...
    auto Err = llvm::handleErrors(
        ValOrErr.takeError(), [&OS, &Result, Kind](latino::ImportError &IE) {
          if (IE.Error == Kind) {
            Result = true;
