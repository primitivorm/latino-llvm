...
struct TestCompiler {
  LLVMContext Context;
  latino::CompilerInstance compiler;
  latino::CodeGenerator *CG = nullptr;
  ...
  void init(const char *TestProgram) {
    ...
    compiler.setTarget(latino::TargetInfo::CreateTargetInfo(
        ...
        std::make_shared<latino::TargetOptions>(compiler.getTargetOpts())));

    const latino::TargetInfo &TInfo = compiler.getTarget();
    ...
    CG = CreateLLVMCodeGen(
        ...
        Context);
    compiler.setASTConsumer(std::unique_ptr<latino::ASTConsumer>(CG));
