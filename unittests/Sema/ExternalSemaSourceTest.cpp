...
class CompleteTypeDiagnoser : public latino::ExternalSemaSource {
public:
...
class DiagnosticWatcher : public latino::DiagnosticConsumer {
...
class NamespaceTypoProvider : public latino::ExternalSemaSource {
...
class FunctionTypoProvider : public latino::ExternalSemaSource {
  std::string CorrectFrom;
  std::string CorrectTo;
  Sema *CurrentSema;
...
class ExternalSemaSourceInstaller : public latino::ASTFrontendAction {
  std::vector<DiagnosticWatcher *> Watchers;
  std::vector<latino::ExternalSemaSource *> Sources;
...
protected:
  ...
  CreateASTConsumer(latino::CompilerInstance &Compiler,
    ...
    return std::make_unique<latino::ASTConsumer>();
  }
...
public:
  void PushSource(latino::ExternalSemaSource *Source) {
...
TEST(ExternalSemaSource, SanityCheck) {
  ...
  ASSERT_TRUE(latino::tooling::runToolOnCodeWithArgs(
...
TEST(ExternalSemaSource, FirstDiagnoserTaken) {
  ...
  Installer->PushSource(&Third);
  std::vector<std::string> Args(1, "-std=c++11");
  ASSERT_FALSE(latino::tooling::runToolOnCodeWithArgs(
