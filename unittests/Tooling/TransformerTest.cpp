...
#include "latino/Tooling/Transformer/Transformer.h"
...
#include "latino/Tooling/Transformer/Stencil.h"
...
static ast_matchers::internal::Matcher<latino::QualType>
isOrPointsTo(const latino::ast_matchers::DeclarationMatcher &TypeMatcher) {
...
protected:
  ...
  latino::ast_matchers::MatchFinder MatchFinder;
...
TEST_F(TransformerTest, AddIncludeQuoted) {
  ...
  std::string Expected = R"cc(#include "latino/OtherLib.h"
...
TEST_F(TransformerTest, OrderedRuleImplicitMatched) {
  ...
  RewriteRule ReplaceF1 = makeRule(
      traverse(latino::TK_AsIs, callExpr(callee(functionDecl(hasName("f1"))))),
      changeTo(cat("REPLACE_F1")));
  RewriteRule ReplaceF2 =
      makeRule(traverse(latino::TK_AsIs,
...
TEST_F(TransformerTest, MultipleFiles) {
  ...
  llvm::Expected<std::string> UpdatedCode =
      latino::tooling::applyAllReplacements(Header,
  ...
  UpdatedCode = latino::tooling::applyAllReplacements(
