...
class TestAction : public latino::ASTFrontendAction {
public:
  /// Takes ownership of TestConsumer.
  explicit TestAction(std::unique_ptr<latino::ASTConsumer> TestConsumer)
...
private:
  std::unique_ptr<latino::ASTConsumer> TestConsumer;
};

class FindTopLevelDeclConsumer : public latino::ASTConsumer {
 public:
  explicit FindTopLevelDeclConsumer(bool *FoundTopLevelDecl)
      : FoundTopLevelDecl(FoundTopLevelDecl) {}
  bool HandleTopLevelDecl(latino::DeclGroupRef DeclGroup) override {
...
namespace {
class FindClassDeclXConsumer : public latino::ASTConsumer {
 public:
  FindClassDeclXConsumer(bool *FoundClassDeclX)
      : FoundClassDeclX(FoundClassDeclX) {}
  bool HandleTopLevelDecl(latino::DeclGroupRef GroupRef) override {
    if (CXXRecordDecl* Record = dyn_cast<latino::CXXRecordDecl>(
            *GroupRef.begin())) {
      if (Record->getName() == "X") {
...
bool FindClassDeclX(ASTUnit *AST) {
  for (std::vector<Decl *>::iterator i = AST->top_level_begin(),
                                     e = AST->top_level_end();
       i != e; ++i) {
    if (CXXRecordDecl* Record = dyn_cast<latino::CXXRecordDecl>(*i)) {
      if (Record->getName() == "X") {
        return true;
      }
    }
...
TEST(ToolInvocation, TestMapVirtualFile) {
  ...
  latino::tooling::ToolInvocation Invocation(
...
struct SkipBodyConsumer : public latino::ASTConsumer {
...
struct SkipBodyAction : public latino::ASTFrontendAction {
...
struct CheckColoredDiagnosticsAction : public latino::ASTFrontendAction {
...
TEST(runToolOnCode, TestResetDiagnostics) {
  ...
  struct ResetDiagnosticAction : public latino::ASTFrontendAction {
    std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &Compiler,
                                                   StringRef) override {
      struct Consumer : public latino::ASTConsumer {
        bool HandleTopLevelDecl(latino::DeclGroupRef D) override {
