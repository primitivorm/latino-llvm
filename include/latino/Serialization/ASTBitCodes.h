...
#ifndef LLVM_LATINO_SERIALIZATION_ASTBITCODES_H
#define LLVM_LATINO_SERIALIZATION_ASTBITCODES_H
...
#include "latino/Serialization/TypeBitCodes.def"
    ...
    enum DeclCode {
      ...
      DECL_LAST = DECL_REQUIRES_EXPR_BODY
    };
...
namespace llvm {
  ...
  template <> struct DenseMapInfo<latino::serialization::DeclarationNameKey> {
    static latino::serialization::DeclarationNameKey getEmptyKey() {
      return latino::serialization::DeclarationNameKey(-1, 1);
    }

    static latino::serialization::DeclarationNameKey getTombstoneKey() {
      return latino::serialization::DeclarationNameKey(-1, 2);
    }

    static unsigned
    getHashValue(const latino::serialization::DeclarationNameKey &Key) {
      return Key.getHash();
    }

    static bool isEqual(const latino::serialization::DeclarationNameKey &L,
                        const latino::serialization::DeclarationNameKey &R) {
      return L == R;
    }
  };
...
#endif // LLVM_LATINO_SERIALIZATION_ASTBITCODES_H
