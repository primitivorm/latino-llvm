...
#ifndef LLVM_LATINO_ANALYSIS_CALLGRAPH_H
#define LLVM_LATINO_ANALYSIS_CALLGRAPH_H
...
// Specialize DenseMapInfo for latino::CallGraphNode::CallRecord.
template <> struct DenseMapInfo<latino::CallGraphNode::CallRecord> {
  static inline latino::CallGraphNode::CallRecord getEmptyKey() {
    return latino::CallGraphNode::CallRecord(
        DenseMapInfo<latino::CallGraphNode *>::getEmptyKey(),
        DenseMapInfo<latino::Expr *>::getEmptyKey());
  }

  static inline latino::CallGraphNode::CallRecord getTombstoneKey() {
    return latino::CallGraphNode::CallRecord(
        DenseMapInfo<latino::CallGraphNode *>::getTombstoneKey(),
        DenseMapInfo<latino::Expr *>::getTombstoneKey());
  }

  static unsigned getHashValue(const latino::CallGraphNode::CallRecord &Val) {
    // NOTE: we are comparing based on the callee only.
    // Different call records with the same callee will compare equal!
    return DenseMapInfo<latino::CallGraphNode *>::getHashValue(Val.Callee);
  }

  static bool isEqual(const latino::CallGraphNode::CallRecord &LHS,
                      const latino::CallGraphNode::CallRecord &RHS) {
    return LHS == RHS;
  }
};

// Graph traits for iteration, viewing.
template <> struct GraphTraits<latino::CallGraphNode*> {
  using NodeType = latino::CallGraphNode;
  using NodeRef = latino::CallGraphNode *;
  using ChildIteratorType = NodeType::iterator;

  static NodeType *getEntryNode(latino::CallGraphNode *CGN) { return CGN; }
  static ChildIteratorType child_begin(NodeType *N) { return N->begin();  }
  static ChildIteratorType child_end(NodeType *N) { return N->end(); }
};

template <> struct GraphTraits<const latino::CallGraphNode*> {
  using NodeType = const latino::CallGraphNode;
  using NodeRef = const latino::CallGraphNode *;
  using ChildIteratorType = NodeType::const_iterator;

  static NodeType *getEntryNode(const latino::CallGraphNode *CGN) { return CGN; }
  static ChildIteratorType child_begin(NodeType *N) { return N->begin();}
  static ChildIteratorType child_end(NodeType *N) { return N->end(); }
};

template <> struct GraphTraits<latino::CallGraph*>
  : public GraphTraits<latino::CallGraphNode*> {
  static NodeType *getEntryNode(latino::CallGraph *CGN) {
    return CGN->getRoot();  // Start at the external node!
  }

  static latino::CallGraphNode *
  CGGetValue(latino::CallGraph::const_iterator::value_type &P) {
    return P.second.get();
  }

  // nodes_iterator/begin/end - Allow iteration over all nodes in the graph
  using nodes_iterator =
      mapped_iterator<latino::CallGraph::iterator, decltype(&CGGetValue)>;

  static nodes_iterator nodes_begin(latino::CallGraph *CG) {
    return nodes_iterator(CG->begin(), &CGGetValue);
  }

  static nodes_iterator nodes_end  (latino::CallGraph *CG) {
    return nodes_iterator(CG->end(), &CGGetValue);
  }

  static unsigned size(latino::CallGraph *CG) { return CG->size(); }
};

template <> struct GraphTraits<const latino::CallGraph*> :
  public GraphTraits<const latino::CallGraphNode*> {
  static NodeType *getEntryNode(const latino::CallGraph *CGN) {
    return CGN->getRoot();
  }
  ...
  // nodes_iterator/begin/end - Allow iteration over all nodes in the graph
  using nodes_iterator =
      mapped_iterator<latino::CallGraph::const_iterator, decltype(&CGGetValue)>;

  static nodes_iterator nodes_begin(const latino::CallGraph *CG) {
    return nodes_iterator(CG->begin(), &CGGetValue);
  }

  static nodes_iterator nodes_end(const latino::CallGraph *CG) {
    return nodes_iterator(CG->end(), &CGGetValue);
  }

  static unsigned size(const latino::CallGraph *CG) { return CG->size(); }
};

} // namespace llvm

#endif // LLVM_LATINO_ANALYSIS_CALLGRAPH_H
