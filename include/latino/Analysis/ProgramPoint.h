...
#ifndef LLVM_LATINO_ANALYSIS_PROGRAMPOINT_H
#define LLVM_LATINO_ANALYSIS_PROGRAMPOINT_H
...
template <> struct DenseMapInfo<latino::ProgramPoint> {

static inline latino::ProgramPoint getEmptyKey() {
  uintptr_t x =
   reinterpret_cast<uintptr_t>(DenseMapInfo<void*>::getEmptyKey()) & ~0x7;
  return latino::BlockEntrance(reinterpret_cast<latino::CFGBlock*>(x), nullptr);
}

static inline latino::ProgramPoint getTombstoneKey() {
  uintptr_t x =
   reinterpret_cast<uintptr_t>(DenseMapInfo<void*>::getTombstoneKey()) & ~0x7;
  return latino::BlockEntrance(reinterpret_cast<latino::CFGBlock*>(x), nullptr);
}

static unsigned getHashValue(const latino::ProgramPoint &Loc) {
  return Loc.getHashValue();
}

static bool isEqual(const latino::ProgramPoint &L,
                    const latino::ProgramPoint &R) {
  return L == R;
}
