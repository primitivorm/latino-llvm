...
public:
  ...
  friend const PartialDiagnostic &operator<<(const PartialDiagnostic &PD,
	  const char* S) {
	  PD.AddTaggedVal(reinterpret_cast<intptr_t>(S), DiagnosticsEngine::ak_c_string);
	  return PD;
  }

  friend const PartialDiagnostic &operator<<(const PartialDiagnostic &PD,
	  IdentifierInfo *II) {
	  PD.AddTaggedVal(reinterpret_cast<intptr_t>(II), DiagnosticsEngine::ak_identifierinfo);
	  return PD;
  }
  ...
  friend inline
	  ...
	  operator<<(const PartialDiagnostic &PD, T *DC) {
		PD.AddTaggedVal(reinterpret_cast<intptr_t>(DC), DiagnosticsEngine::ak_declcontext);
	}
