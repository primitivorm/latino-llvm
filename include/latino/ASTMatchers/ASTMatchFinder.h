...
#ifndef LLVM_LATINO_ASTMATCHERS_ASTMATCHFINDER_H
#define LLVM_LATINO_ASTMATCHERS_ASTMATCHFINDER_H
...
public:
  ...
  struct MatchResult {
    MatchResult(const BoundNodes &Nodes, latino::ASTContext *Context);
    ...
    /// @{
    latino::ASTContext * const Context;
    latino::SourceManager * const SourceManager;
    /// @}
  };
  ...
  std::unique_ptr<latino::ASTConsumer> newASTConsumer();
  ...
  template <typename T> void match(const T &Node, ASTContext &Context) {
    match(latino::DynTypedNode::create(Node), Context);
  }
  void match(const latino::DynTypedNode &Node, ASTContext &Context);
  /// @}
