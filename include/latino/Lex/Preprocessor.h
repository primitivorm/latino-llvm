...
class Preprocessor {
  ...
  llvm::unique_function<void(const latino::Token &)> OnToken;
  ...
  /// control flow (in latino::ParseAST()), make it convenient to put here.
  ///
...
public:
  ...
  void setTokenWatcher(llvm::unique_function<void(const latino::Token &)> F) {
    OnToken = std::move(F);
  }
