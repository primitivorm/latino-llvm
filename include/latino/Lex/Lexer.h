...
public:
  ...
  */
  ...
  std::string getSpelling(const Token &Tok, const SourceManager &SourceMgr,
                          const LangOptions &LangOpts, bool *Invalid = nullptr);
 ...
 macro
    /// responsible for its immediate expansion. It looks through any
 intervening
    ...
    /// that SourceManager.
    ///
    /// This differs from Lexer::getImmediateMacroName in that any macro
 argument
    ...
    static StringRef getImmediateMacroNameForDiagnostics(
        SourceLocation Loc, const SourceManager &SM, const LangOptions
 &LangOpts);

    /// Compute the preamble of the given file.
    ///
    /// The preamble of a file contains the initial comments, include
 directives,
...
private:
  ...
  */
  ...
  void codeCompleteIncludedFile(const char *PathStart,
    ...
    ///         invalid.
    uint32_t tryReadUCN(const char *&StartPtr, const char *SlashLoc, Token
  *Result);
    ...
    ///               buffer containing the '\'. Updated to point past the end
  of
    ///               the UCN on success.
