...
#ifndef LLVM_LATINO_SEMA_OWNERSHIP_H
#define LLVM_LATINO_SEMA_OWNERSHIP_H
...
namespace llvm {
template <class T> struct PointerLikeTypeTraits<latino::OpaquePtr<T>> {
  static constexpr int NumLowBitsAvailable = 0;

  static inline void *getAsVoidPointer(latino::OpaquePtr<T> P) {
    // FIXME: Doesn't work? return P.getAs< void >();
    return P.getAsOpaquePtr();
  }

  static inline latino::OpaquePtr<T> getFromVoidPointer(void *P) {
    return latino::OpaquePtr<T>::getFromOpaquePtr(P);
  }
};
} // namespace llvm
...
public:
  ...
  static ActionResult getFromOpaquePtr(void *P) {
    ActionResult Result;
...
// } inline StmtResult StmtError(const DiagnosticBuilder &) { return
// StmtError(); }
