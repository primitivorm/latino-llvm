...
#include "latino/Sema/TemplateDeduction.h"
...
static bool evaluateTypeTrait(Sema &S, TypeTrait Kind, SourceLocation KWLoc,
  ...
  switch (Kind) {
  case latino::BTT_ReferenceBindsToTemporary:
  case latino::TT_IsConstructible:
  case latino::TT_IsNothrowConstructible:
  case latino::TT_IsTriviallyConstructible: {
    ...
    if (Kind == latino::TT_IsConstructible)
      return true;

    if (Kind == latino::BTT_ReferenceBindsToTemporary) {
      if (!T->isReferenceType())
        return false;
    ...
    if (Kind == latino::TT_IsNothrowConstructible)
    ...
    if (Result.isInvalid() || Result.get() != E) {
      ...
      if (Result.isInvalid() && RecoverUncorrectedTypos) {
        struct TyposReplace : TreeTransform<TyposReplace> {
          ...
          ExprResult TransformTypoExpr(latino::TypoExpr *E) {
