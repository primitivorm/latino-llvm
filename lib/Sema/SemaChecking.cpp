...
#include "latino/Basic/SyncScope.h"
...
static bool SemaBuiltinAlignment(Sema &S, CallExpr *TheCall, unsigned ID) {
  ...
  latino::Expr *Source = TheCall->getArg(0);
  ...
  latino::Expr *AlignOp = TheCall->getArg(1);
...
Sema::CheckBuiltinFunctionCall(FunctionDecl *FDecl, unsigned BuiltinID,
  ...
  case Builtin::BI__builtin_memcpy_inline: {
    latino::Expr *SizeOp = TheCall->getArg(2);
        ...
        if (ConversionStatus != llvm::APFloat::opOK) {
          ...
          S.DiagRuntimeBehavior(
              ...
              S.PDiag(diag::warn_impcast_integer_float_precision_constant)
                  ...
                  << E->getSourceRange() << latino::SourceRange(CC));
        }
      } else {
