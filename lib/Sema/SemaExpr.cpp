...
static QualType checkVectorShift(Sema &S, ExprResult &LHS, ExprResult &RHS,
  ...
  if (!LHSVecTy) {
    ...
    if (!S.LangOpts.OpenCL && !S.LangOpts.ZVector) {
      const BuiltinType *LHSBT = LHSEleType->getAs<latino::BuiltinType>();
      const BuiltinType *RHSBT = RHSEleType->getAs<latino::BuiltinType>();
      if (LHSBT != RHSBT &&
...
#if __has_cpp_attribute(latino::lifetimebound)
  [[latino::lifetimebound]]
#endif
#endif
  {
