...
#include "latino/AST/QualTypeNames.h"
...
SimplifiedTypeClass latino::getSimplifiedTypeClass(CanQualType T) {
...
QualType latino::getDeclUsageType(ASTContext &C, const NamedDecl *ND) {
  ND = ND->getUnderlyingDecl();
...
static OverloadCompare compareOverloads(const CXXMethodDecl &Candidate,
  ...
  if (CandidateRef != IncumbentRef) {
    ...
    if (ObjectKind == latino::VK_XValue)
...
static void AddOrdinaryNameResults(Sema::ParserCompletionContext CCC, Scope *S,
  ...
  case Sema::PCC_Statement: {
    ...
    if (ReturnType.isNull() || ReturnType->isVoidType()) {
      ...
      // "return expression ;"
      Builder.AddTypedTextChunk("return");
      Builder.AddChunk(latino::CodeCompletionString::CK_HorizontalSpace);
      Builder.AddPlaceholderChunk("expression");
      Builder.AddChunk(CodeCompletionString::CK_SemiColon);
      Results.AddResult(Result(Builder.TakeString()));
...
static void AddResultTypeChunk(ASTContext &Context,
  ...
  // } 
    ...
    T = latino::TypeName::getFullyQualifiedType(T, Context);
...
const RawComment *latino::getCompletionComment(const ASTContext &Ctx,
                                              const NamedDecl *ND) {
  if (!ND)
    return nullptr;
...
const RawComment *latino::getParameterComment(
...
unsigned latino::getMacroUsagePriority(StringRef MacroName,
                                      const LangOptions &LangOpts,
...
CXCursorKind latino::getCursorKindForDecl(const Decl *D) {
  if (!D)
...
static void AddLambdaCompletion(ResultBuilder &Results,
  ...
  Completion.AddChunk(latino::CodeCompletionString::CK_HorizontalSpace);
...
public:
  ...
  struct Member {
    ...
    CodeCompletionString *render(Sema &S, CodeCompletionAllocator &Alloc,
      ...
      if (ArgTypes) {
        B.AddChunk(latino::CodeCompletionString::CK_LeftParen);
        bool First = true;
        for (QualType Arg : *ArgTypes) {
          if (First)
            First = false;
          else {
            B.AddChunk(latino::CodeCompletionString::CK_Comma);
            B.AddChunk(latino::CodeCompletionString::CK_HorizontalSpace);
          }
          B.AddPlaceholderChunk(Alloc.CopyString(
              Arg.getAsString(getCompletionPrintingPolicy(S))));
        }
        B.AddChunk(latino::CodeCompletionString::CK_RightParen);
      }
      return B.TakeString();
    }
  };
