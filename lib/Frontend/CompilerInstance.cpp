...
#include "latino/Frontend/LogDiagnosticPrinter.h"
...
#include "latino/Frontend/VerifyDiagnosticConsumer.h"
...
static void SetupSerializedDiagnostics(DiagnosticOptions *DiagOpts,
  ...
  auto SerializedConsumer =
      latino::serialized_diags::create(OutputFile, DiagOpts);
...
CompilerInstance::loadModule(SourceLocation ImportLoc,
  ...
  if (Path.size() > 1) {
    for (unsigned I = 1, N = Path.size(); I != N; ++I) {
      ...
      latino::Module *Sub = Module->findSubmodule(Name);
      ...
      if (!Sub) {
        ...
        for (latino::Module::submodule_iterator J = Module->submodule_begin(),
  ...
  latino::Module *TopModule = Module->getTopLevelModule();
