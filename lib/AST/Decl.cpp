...
Decl *latino::getPrimaryMergedDecl(Decl *D) {
...
bool FunctionDecl::isTemplateInstantiation() const {
  ...
  // specialization kind?)
  return latino::isTemplateInstantiation(getTemplateSpecializationKind());
}
...
FunctionDecl::getTemplateInstantiationPattern(bool ForDefinition) const {
  ...
  if (MemberSpecializationInfo *Info = getMemberSpecializationInfo()) {
    if (ForDefinition &&
        !latino::isTemplateInstantiation(Info->getTemplateSpecializationKind()))
      return nullptr;
    return getDefinitionOrSelf(cast<FunctionDecl>(Info->getInstantiatedFrom()));
  }

  if (ForDefinition &&
      !latino::isTemplateInstantiation(getTemplateSpecializationKind()))
    return nullptr;
