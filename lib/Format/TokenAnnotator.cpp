...
static bool isKeywordWithCondition(const FormatToken &Tok) {
  return Tok.isOneOf(tok::kw_si, tok::kw_desde, tok::kw_mientras, tok::kw_elegir,
                     tok::kw_constexpr, tok::kw_atrapar);
}
...
private:
  bool parseAngle() {
    ...
    Contexts.back().InTemplateArgument =
        Left->Previous && Left->Previous->Tok.isNot(tok::kw_plantilla);
    ...
    while (CurrentToken) {
      if (CurrentToken->is(tok::greater)) {
        ...
        if (Style.Language == FormatStyle::LK_TextProto ||
            (Style.Language == FormatStyle::LK_Proto && Left->Previous &&
             Left->Previous->is(/*TT_SelectorName,*/ TT_DictLiteral)))
      ...
      if (CurrentToken->Previous->isOneOf(tok::pipepipe, tok::ampamp) &&
          ...
          !Line.startsWith(tok::kw_plantilla))
        return false;
  ...
  bool parseParens(bool LookForDecls = false) {
    ...
    //     Left->Previous && Left->Previous->is(tok::kw_desde);
  ...
  bool isCpp11AttributeSpecifier(const FormatToken &Tok) {
    ...
    if (AttrTok->startsSequence(tok::kw_usar, tok::identifier, tok::colon))
      return true;
  ...
  bool consumeToken() {
    ...
    case tok::colon:
      ...
      if (Style.Language == FormatStyle::LK_JavaScript) {
        if (Contexts.back().ColonIsForRangeExpr || // colon in for loop
            (Contexts.size() == 1 &&               // switch/case labels
             !Line.First->isOneOf(tok::kw_enum, tok::kw_caso)) ||
      ...
      if (Contexts.back().ColonIsDictLiteral ||
        ...
        else if (Prev->is(tok::kw_intentar)) {
    ...
    case tok::kw_mientras:
      if (Tok->is(tok::kw_si) && CurrentToken &&
    ...
    case tok::kw_desde:
      ...
      break;
    case tok::l_paren:
      // When faced with 'operator()()', the kw_operador handler incorrectly
    ...
    case tok::kw_operador:
      ...
      while (CurrentToken &&
        ...
        if (CurrentToken && CurrentToken->is(tok::comma) &&
            CurrentToken->Previous->isNot(tok::kw_operador))
          break;
    ...
    case tok::comma:
      if (Contexts.back().InCtorInitializer)
        ...
        Tok->setType(TT_InheritanceComma);
               ...
               (Contexts.size() == 1 || Line.startsWith(tok::kw_desde))) {
...
private:
    ...
    if (Current.getPrecedence() == prec::Assignment &&
        ...
        /*!(Style.Language == FormatStyle::LK_JavaScript &&
          (Line.startsWith(Keywords.kw_type, tok::identifier) ||
           Line.startsWith(tok::kw_exportar, Keywords.kw_type,
                           tok::identifier))) &&*/
        (!Current.Previous || Current.Previous->isNot(tok::kw_operador))) {
      Contexts.back().IsExpression = true;
        ...
        for (FormatToken *Previous = Current.Previous;
          ...
          if (Previous->isOneOf(TT_BinaryOperator, TT_UnaryOperator) &&
            ...
            Previous->setType(TT_PointerOrReference);
               ...
               (!Current.Previous || !Current.Previous->is(tok::kw_operador))) {
      Contexts.back().IsExpression = true;
    } else if (Current.isOneOf(tok::kw_ret, tok::kw_lanzar)) {
      Contexts.back().IsExpression = true;
      ...
      if (Line.MustBeDeclaration && !Contexts.front().InCtorInitializer)
        Contexts.back().IsExpression = false;
    } else if (Current.is(tok::kw_nuevo)) {
      Contexts.back().CanBeExpression = false;
    } else if (Current.is(tok::semi) ||
               (Current.is(tok::exclaim) && Current.Previous &&
                !Current.Previous->is(tok::kw_operador))) {
  ...
  void determineTokenType(FormatToken &Current) {
    ...
    //              tok::kw_contexto, tok::r_paren, tok::r_square, tok::r_brace,
  ...
  bool isStartOfName(const FormatToken &Tok) {
    ...
    if (PreviousNotConst->is(TT_TemplateCloser))
      return PreviousNotConst && PreviousNotConst->MatchingParen &&
             ...
             PreviousNotConst->MatchingParen->Previous->isNot(tok::kw_plantilla);
  ...
  bool rParenEndsCast(const FormatToken &Tok) {
    ...
    if (LeftOfParens) {
      ...
      if (LeftOfParens->Tok.getIdentifierInfo() &&
          !LeftOfParens->isOneOf(/*Keywords.kw_in,*/ tok::kw_ret, tok::kw_caso,
                                 tok::kw_borrar))
        return false;
    ...
    if (Tok.Next->isOneOf(tok::identifier, tok::kw_mi))
      return true;
  ...
  TokenType determineStarAmpUsage(const FormatToken &Tok, bool IsExpression,
    ...
    if (PrevToken->isOneOf(tok::l_paren, tok::l_square, tok::l_brace,
                           ...
                           tok::equal, tok::kw_borrar, tok::kw_sizeof,
                           tok::kw_lanzar) ||
    ...
    if (NextToken->is(tok::l_square) && NextToken->isNot(TT_LambdaLSquare))
      return TT_PointerOrReference;
    if (NextToken->is(tok::kw_operador) && !IsExpression)
      return TT_PointerOrReference;
  ...
  TokenType determinePlusMinusCaretUsage(const FormatToken &Tok) {
    ...
    if (PrevToken->isOneOf(tok::equal, tok::l_paren, tok::comma, tok::l_square,
                           tok::question, tok::colon, tok::kw_ret,
                           tok::kw_caso, tok::at, tok::l_brace, tok::kw_lanzar,
  ...
  void parse(int Precedence = 0) {
    ...
    while (Current && (Current->is(tok::kw_ret) ||
                       (Current->is(tok::colon) &&
                        Current->is(/*TT_ObjCMethodExpr,*/ TT_DictLiteral))))
      next();
  ...
  auto skipOperatorName = [](const FormatToken *Next) -> const FormatToken * {
    for (; Next; Next = Next->Next) {
      ...
      if (Next->is(TT_OverloadedOperator))
        continue;
      if (Next->isOneOf(tok::kw_nuevo, tok::kw_borrar)) {
        // For 'new[]' and 'delete[]'.
        if (Next->Next &&
...
unsigned TokenAnnotator::splitPenalty(const AnnotatedLine &Line,
  ...
  if (Right.isOneOf(TT_StartOfName, TT_FunctionDeclarationName) ||
      Right.is(tok::kw_operador)) {
    if (Line.startsWith(tok::kw_desde) && Right.PartOfMultiVariableDeclStmt)
      return 3;
  ...
  if (Line.startsWith(tok::kw_desde) && Left.is(tok::equal))
    return 4;
  ...
  if (Left.is(tok::l_paren) && Left.Previous &&
      (Left.Previous->is(tok::kw_desde) || Left.Previous->isIf()))
    return 1000;
...
bool TokenAnnotator::spaceRequiredBetween(const AnnotatedLine &Line,
  ...
  if (Right.is(tok::less) && Left.is(tok::kw_plantilla))
            ...
            (!Left.isOneOf(TT_PointerOrReference, tok::l_paren) &&
             (Style.PointerAlignment != FormatStyle::PAS_Left ||
              (Line.IsMultiVariableDeclStmt &&
               (Left.NestingLevel == 0 ||
                (Left.NestingLevel == 1 && Line.First->is(tok::kw_desde)))))));
  }
  ...
  if (Right.isOneOf(tok::star, tok::amp, tok::ampamp)) {
    ...
    while (Previous && !Previous->is(tok::kw_operador)) {
    ...
    if (Previous &&
        (Previous->endsSequence(tok::kw_operador) ||
         Previous->endsSequence(tok::kw_const, tok::kw_operador) /*||
         Previous->endsSequence(tok::kw_volatile, tok::kw_operador)*/))
...
bool TokenAnnotator::spaceRequiredBefore(const AnnotatedLine &Line,
  ...
  if (Style.isCpp()) {
    ...
    if (Left.is(tok::kw_mi) && Right.is(tok::l_square))
      return false;

    // No space between 'new' and '('
    if (Left.is(tok::kw_nuevo) && Right.is(tok::l_paren))
      return false;
    ...
    if (Right.is(tok::l_paren))
      if (Left.is/*OneOf*/(tok::kw_usar/*, Keywords.kw_async, Keywords.kw_when,
                       Keywords.kw_lock*/))
    ...
    if (Right.is(tok::l_paren)) {
      ...
      if (Left.isOneOf(tok::kw_lanzar, /*Keywords.kw_await, Keywords.kw_typeof,*/
                       tok::kw_void))
        return true;
    }
    ...
    //   return true;
    if (Left.is/*OneOf*/(tok::kw_desde/*, Keywords.kw_as*/) && Left.Previous &&
    ...
    if (Left.is(TT_JsTypeOperator) || Right.is(TT_JsTypeOperator))
      return false;
    if ((Left.is(tok::l_brace) || Right.is(tok::r_brace)) &&
        Line.First->is/*OneOf*/(/*Keywords.kw_import,*/ tok::kw_exportar))
      return false;
  ...
  if (Right.is(tok::colon)) {
    if (Line.First->isOneOf(tok::kw_caso, tok::kw_otro) ||
  ...
  if (Left.is(TT_CastRParen))
    return Style.SpaceAfterCStyleCast ||
           Right.is(TT_BinaryOperator/*, TT_SelectorName*/);
  ...
  //     (Left.is(tok::identifier) || Left.is(tok::kw_mi)))
  //   return false;
...
bool TokenAnnotator::canBreakBefore(const AnnotatedLine &Line,
  ...
  if (Style.isCSharp()) {
    ...
    if (Left.is(Keywords.kw_declare) &&
        Right.isOneOf(Keywords.kw_modulo, tok::kw_contexto,
                      Keywords.kw_function, tok::kw_clase, tok::kw_enum,
      ...
      return false;
    if (Left.isOneOf(Keywords.kw_modulo, tok::kw_contexto) &&
  ...
  if (Left.isOneOf(TT_TemplateCloser, TT_UnaryOperator) ||
      Left.is(tok::kw_operador))
    return false;
  if (Left.is(tok::equal) && !Right.isOneOf(tok::kw_otro, tok::kw_borrar) &&
