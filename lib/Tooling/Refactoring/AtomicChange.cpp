...
LLVM_YAML_IS_SEQUENCE_VECTOR(latino::tooling::AtomicChange)
...
struct NormalizedAtomicChange {
  ...
  NormalizedAtomicChange(const llvm::yaml::IO &,
                         const latino::tooling::AtomicChange &E)
  ...
  latino::tooling::AtomicChange denormalize(const llvm::yaml::IO &) {
  ...
  std::vector<latino::tooling::Replacement> Replaces;
};
} // anonymous namespace
...
/// Specialized MappingTraits to describe how an AtomicChange is
/// (de)serialized.
template <> struct MappingTraits<latino::tooling::AtomicChange> {
  static void mapping(IO &Io, latino::tooling::AtomicChange &Doc) {
    MappingNormalization<NormalizedAtomicChange, latino::tooling::AtomicChange>
        Keys(Io, Doc);
...
getRangesForFormating(llvm::StringRef Code, unsigned ColumnLimit,
                      ...
                      const latino::tooling::Replacements &Replaces) {
  // kNone suppresses formatting entirely.
  if (Format == ApplyChangesSpec::kNone)
    return {};
  std::vector<latino::tooling::Range> Ranges;
  ...
  int Offset = 0;
  for (const latino::tooling::Replacement &R : Replaces) {
...
createReplacementsForHeaders(llvm::StringRef FilePath, llvm::StringRef Code,
  ...
  return latino::format::cleanupAroundReplacements(Code, HeaderReplacements,
                                                  Style);
}
...
AtomicChange::AtomicChange(std::string Key, std::string FilePath,
                           ...
                           std::vector<std::string> InsertedHeaders,
                           std::vector<std::string> RemovedHeaders,
                           latino::tooling::Replacements Replaces)
