...
#include "latino/Tooling/Refactoring/Rename/USRLocFinder.h"
...
#include "latino/Tooling/Core/Lookup.h"
#include "latino/Tooling/Refactoring/RecursiveSymbolVisitor.h"
#include "latino/Tooling/Refactoring/Rename/SymbolName.h"
#include "latino/Tooling/Refactoring/Rename/USRFinder.h"
...
bool IsValidEditLoc(const latino::SourceManager& SM, latino::SourceLocation Loc) {
  if (Loc.isInvalid())
    return false;
  const latino::FullSourceLoc FullLoc(Loc, SM);
  std::pair<latino::FileID, unsigned> FileIdAndOffset =
...
SourceLocation StartLocationForType(TypeLoc TL) {
  ...
  if (auto ElaboratedTypeLoc = TL.getAs<latino::ElaboratedTypeLoc>()) {
...
private:
  ...
  const NamedDecl *getSupportedDeclFromTypeLoc(TypeLoc Loc) {
    if (const auto* TT = Loc.getType()->getAs<latino::TypedefType>())
      return TT->getDecl();
  ...
  for (const auto &RenameInfo : Finder.getRenameInfos()) {
    ...
    if (RenameInfo.IgnorePrefixQualifers) {
      ...
      if (RenameInfo.FromDecl && RenameInfo.Context) {
        if (!llvm::isa<latino::TranslationUnitDecl>(
