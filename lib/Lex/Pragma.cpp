...
#include "latino/Lex/LiteralSupport.h"
...
void Preprocessor::HandlePragmaModuleBuild(Token &Tok) {
  ...
  while (true) {
    ...
    CurLexer->Lex(Tok);
    if (TryConsumeIdentifier("pragma") && TryConsumeIdentifier("latino") &&
        TryConsumeIdentifier("module")) {
      if (TryConsumeIdentifier("build"))
        // #pragma latino module build -> entering a nested module build.
        ++NestingLevel;
      else if (TryConsumeIdentifier("endbuild")) {
        // #pragma latino module endbuild -> leaving a module build.
        if (--NestingLevel == 0)
          break;
      }
...
struct PragmaDebugHandler : public PragmaHandler {
  ...
  void HandlePragma(Preprocessor &PP, PragmaIntroducer Introducer,
    ...
    if (II->isStr("assert")) {
      ...
      if (!PP.getPreprocessorOpts().DisablePragmaDebugCrash)
        llvm::report_fatal_error("#pragma latino __debug llvm_fatal_error");
    } else if (II->isStr("llvm_unreachable")) {
      if (!PP.getPreprocessorOpts().DisablePragmaDebugCrash)
        llvm_unreachable("#pragma latino __debug llvm_unreachable");
  ...
  void HandleCaptured(Preprocessor &PP) {
    ...
    if (Tok.isNot(tok::eod)) {
      PP.Diag(Tok, diag::ext_pp_extra_tokens_at_eol)
        << "pragma latino __debug captured";
      return;
    }
...
public:
  ...
  void HandlePragma(Preprocessor &PP, PragmaIntroducer Introducer,
    ...
    if (Group == "everything") {
      // Special handling for pragma latino diagnostic ... "-Weverything".
...
/// "\#pragma warning(...)".  MSVC's diagnostics do not map cleanly to latino's
...
/// Handle the latino \#pragma module import extension. The syntax is:
/// \code
///   #pragma latino module import some.module.name
/// \endcode
...
/// Handle the latino \#pragma module begin extension. The syntax is:
/// \code
///   #pragma latino module begin some.module.name
///   ...
///   #pragma latino module end
/// \endcode
...
/// Handle the latino \#pragma module end extension.
...
/// Handle the latino \#pragma module build extension.
...
/// Handle the latino \#pragma module load extension.
...
///   \#pragma latino arc_cf_code_audited begin/end
...
///   \#pragma latino assume_nonnull begin/end
...
void Preprocessor::RegisterBuiltinPragmas() {
  ...
  AddPragmaHandler("GCC", new PragmaMessageHandler(PPCallbacks::PMK_Error,
                                                   "GCC"));
  // #pragma latino ...
  AddPragmaHandler("latino", new PragmaPoisonHandler());
  AddPragmaHandler("latino", new PragmaSystemHeaderHandler());
  AddPragmaHandler("latino", new PragmaDebugHandler());
  AddPragmaHandler("latino", new PragmaDependencyHandler());
  AddPragmaHandler("latino", new PragmaDiagnosticHandler("latino"));
  AddPragmaHandler("latino", new PragmaARCCFCodeAuditedHandler());
  AddPragmaHandler("latino", new PragmaAssumeNonNullHandler());

  // #pragma latino module ...
  auto *ModuleHandler = new PragmaNamespace("module");
  AddPragmaHandler("latino", ModuleHandler);
...
void Preprocessor::IgnorePragmas() {
  ...
  AddPragmaHandler("latino", new EmptyPragmaHandler());
}
