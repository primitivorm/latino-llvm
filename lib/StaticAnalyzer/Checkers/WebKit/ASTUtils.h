...
#ifndef LLVM_LATINO_ANALYZER_WEBKIT_ASTUTILS_H
#define LLVM_LATINO_ANALYZER_WEBKIT_ASTUTILS_H
...
std::pair<const latino::Expr *, bool>
tryToFindPtrOrigin(const latino::Expr *E, bool StopAtFirstRefCountedObj);
...
bool isASafeCallArg(const latino::Expr *E);

/// \returns name of AST node or empty string.
template <typename T> std::string safeGetName(const T *ASTNode) {
  const auto *const ND = llvm::dyn_cast_or_null<latino::NamedDecl>(ASTNode);
  if (!ND)
    return "";
