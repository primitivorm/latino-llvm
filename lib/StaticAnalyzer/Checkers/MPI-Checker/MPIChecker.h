...
#ifndef LLVM_LATINO_LIB_STATICANALYZER_CHECKERS_MPICHECKER_MPICHECKER_H
#define LLVM_LATINO_LIB_STATICANALYZER_CHECKERS_MPICHECKER_MPICHECKER_H

#include "MPIBugReporter.h"
#include "MPITypes.h"
#include "latino/StaticAnalyzer/Checkers/MPIFunctionClassifier.h"
...
public:
  ...
  void checkDoubleNonblocking(const latino::ento::CallEvent &PreCallEvent,
                              latino::ento::CheckerContext &Ctx) const;
  ...
  ///
  /// \param PreCallEvent MPI call to verify
  void checkUnmatchedWaits(const latino::ento::CallEvent &PreCallEvent,
                           latino::ento::CheckerContext &Ctx) const;
  ...
  void checkMissingWaits(latino::ento::SymbolReaper &SymReaper,
                         latino::ento::CheckerContext &Ctx) const;
...
private:
  ...
  void allRegionsUsedByWait(
      llvm::SmallVector<const latino::ento::MemRegion *, 2> &ReqRegions,
      const latino::ento::MemRegion *const MR, const latino::ento::CallEvent &CE,
      latino::ento::CheckerContext &Ctx) const;
  ...
  const latino::ento::MemRegion *
  topRegionUsedByWait(const latino::ento::CallEvent &CE) const;
