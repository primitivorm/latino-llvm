...
#include "latino/Driver/XRayArgs.h"
...
static void CheckPreprocessingOptions(const Driver &D, const ArgList &Args) {
  if (Arg *A =
          Args.getLastArg(latino::driver::options::OPT_C, options::OPT_CC)) {
...
void Clang::DumpCompilationDatabase(Compilation &C, StringRef Filename,
  ...
  if (!CompilationDatabase) {
    ...
    auto File = std::make_unique<llvm::raw_fd_ostream>(Filename, EC,
                                                        llvm::sys::fs::OF_Text);
    if (EC) {
      D.Diag(latino::diag::err_drv_compilationdatabase) << Filename
                                                       << EC.message();
      return;
    }
  ...
  for (const Arg *A : Args) {
    ...
    case options::OPT_ffp_model_EQ: {
      ...
      if (OFastEnabled && !Val.equals("fast")) {
        ...
        D.Diag(latino::diag::warn_drv_overriding_flag_option)
...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (Args.hasFlag(options::OPT_fdiscard_value_names,
    ...
    if (Args.hasArg(options::OPT_fdiscard_value_names) &&
        (std::any_of(Inputs.begin(), Inputs.end(),
                     [](const latino::driver::InputInfo &II) {
  ...
  if (ImplyVCPPCXXVer) {
    ...
    if (const Arg *StdArg = Args.getLastArg(options::OPT__SLASH_std)) {
      ...
      if (LanguageStandard.empty())
        D.Diag(latino::diag::warn_drv_unused_argument)
  ...
  if (Output.getType() == types::TY_Dependencies) {
    ...
    if (Output.getType() == latino::driver::types::TY_IFS_CPP ||
        Output.getType() == latino::driver::types::TY_IFS) {
...
static EHFlags parseClangCLEHFlags(const Driver &D, const ArgList &Args) {
  ...
  for (auto EHVal : EHArgs) {
    for (size_t I = 0, E = EHVal.size(); I != E; ++I) {
      ...
      D.Diag(latino::diag::err_drv_invalid_value) << "/EH" << EHVal;
      break;
    }
  }
...
void Clang::AddClangCLArgs(const ArgList &Args, types::ID InputType,
 ...
 if (Args.hasFlag(options::OPT__SLASH_Zc_dllexportInlines_,
                  ...
                  false)) {
   if (Args.hasArg(options::OPT__SLASH_fallback)) {
     D.Diag(latino::diag::err_drv_dllexport_inlines_and_fallback);
   } else {
