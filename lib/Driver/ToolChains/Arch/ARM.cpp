...
void arm::getARMArchCPUFromArgs(const ArgList &Args, llvm::StringRef &Arch,
  ...
  if (const Arg *A = Args.getLastArg(latino::driver::options::OPT_mcpu_EQ))
    CPU = A->getValue();
...
static void getARMHWDivFeatures(const Driver &D, const Arg *A,
  ...
  if (!llvm::ARM::getHWDivFeatures(HWDivID, Features))
    D.Diag(latino::diag::err_drv_clang_unsupported) << A->getAsString(Args);
}
...
void arm::getARMTargetFeatures(const Driver &D, const llvm::Triple &Triple,
  ...
  if (WaArch) {
    ...
    // FIXME: Set Arch.
    D.Diag(latino::diag::warn_drv_unused_argument) << WaArch->getAsString(Args);
  } else if (ArchArg) {
  ...
  if (WaFPU) {
    ...
    if (!llvm::ARM::getFPUFeatures(FPUID, Features))
      D.Diag(latino::diag::err_drv_clang_unsupported)
  ...
  if (CryptoIt != Features.rend()) {
    if (CryptoIt->take_front() == "+") {
      ...
      if (llvm::ARM::parseArchVersion(ArchSuffix) >= 8 &&
        ...
        D.Diag(latino::diag::warn_target_unsupported_extension)
            << "crypto"
