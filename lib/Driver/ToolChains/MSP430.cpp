...
#include "latino/Driver/Multilib.h"
...
#include "latino/Basic/MSP430Target.def"
      .Default(false);
}
...
void msp430::getMSP430TargetFeatures(const Driver &D, const ArgList &Args,
  ...
  if (HWMult == "auto") {
    ...
    if (!MCU)
      D.Diag(latino::diag::warn_drv_msp430_hwmult_no_device);
  ...
  if (MCU && SupportedHWMult == "none")
    D.Diag(latino::diag::warn_drv_msp430_hwmult_unsupported) << HWMult;
  if (MCU && HWMult != SupportedHWMult)
    D.Diag(latino::diag::warn_drv_msp430_hwmult_mismatch)
