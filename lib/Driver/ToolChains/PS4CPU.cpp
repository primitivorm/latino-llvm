...
using latino::driver::tools::AddLinkerInputs;
...
toolchains::PS4CPU::PS4CPU(const Driver &D, const llvm::Triple &Triple,
  ...
  if (Args.hasArg(latino::driver::options::OPT_static))
    D.Diag(latino::diag::err_drv_unsupported_opt_for_target) << "-static"
                                                            << "PS4";
  ...
  if (const char *EnvValue = getenv("SCE_ORBIS_SDK_DIR")) {
    if (!llvm::sys::fs::exists(EnvValue))
      getDriver().Diag(latino::diag::warn_drv_ps4_sdk_dir) << EnvValue;
    PS4SDKDir = EnvValue;
  } else {
  ...
  if (const Arg *A = Args.getLastArg(options::OPT_isysroot)) {
    ...
    if (!llvm::sys::fs::exists(PrefixDir))
      getDriver().Diag(latino::diag::warn_missing_sysroot) << PrefixDir;
  } else
  ...
  if (!Args.hasArg(options::OPT_nostdinc) &&
    ...
    getDriver().Diag(latino::diag::warn_drv_unable_to_find_directory_expected)
...
void toolchains::PS4CPU::addClangTargetOptions(
  ...
  if (DriverArgs.hasArg(options::OPT_fuse_init_array)) {
    ...
    getDriver().Diag(latino::diag::err_drv_unsupported_opt_for_target)
