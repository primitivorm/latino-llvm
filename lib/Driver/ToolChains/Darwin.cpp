...
ToolChain::RuntimeLibType DarwinClang::GetRuntimeLibType(
  ...
  if (Arg* A = Args.getLastArg(options::OPT_rtlib_EQ)) {
    ...
    if (Value != "compiler-rt")
      getDriver().Diag(latino::diag::err_drv_unsupported_rtlib_for_platform)
          << Value << "darwin";
  }
...
void Darwin::AddDeploymentTarget(DerivedArgList &Args) const {
  ...
  if (const Arg *A = Args.getLastArg(options::OPT_isysroot)) {
    ...
    if (!getVFS().exists(A->getValue()))
      getDriver().Diag(latino::diag::warn_missing_sysroot) << A->getValue();
  } else {
    ...
    if (OSVersionArgTarget) {
      ...
      if (OSTarget->getPlatform() != OSVersionArgTarget->getPlatform() ||
        ...
        if (OSTarget->getPlatform() == OSVersionArgTarget->getPlatform() &&
          ...
          getDriver().Diag(latino::diag::warn_drv_overriding_flag_option)
...
DerivedArgList *MachO::TranslateArgs(const DerivedArgList &Args,
  ...
  if (!BoundArch.empty()) {
    ...
    const Option MArch = Opts.getOption(latino::driver::options::OPT_march_EQ);
...
Darwin::TranslateArgs(const DerivedArgList &Args, StringRef BoundArch,
  ...
  if (Type == ToolChain::CST_Libcxx) {
    ...
    if (where != StringRef()) {
      getDriver().Diag(latino::diag::err_drv_invalid_libcxx_deployment) << where;
    }
  }
  ...
  if ((Arch == llvm::Triple::arm || Arch == llvm::Triple::thumb)) {
    if (Args.hasFlag(options::OPT_fomit_frame_pointer,
      ...
      getDriver().Diag(latino::diag::warn_drv_unsupported_opt_for_target)
