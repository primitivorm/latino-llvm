...
#include "Linux.h"
#include "latino/Config/config.h" // for GCC_INSTALL_PREFIX
...
void tools::gcc::Common::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  for (const auto &II : Inputs) {
    ...
    if (types::isLLVMIR(II.getType()))
      D.Diag(latino::diag::err_drv_no_linker_llvm_support)
...
bool latino::driver::findMIPSMultilibs(const Driver &D,
...
static llvm::StringRef getGCCToolchainDir(const ArgList &Args,
  ...
  const Arg *A = Args.getLastArg(latino::driver::options::OPT_gcc_toolchain);
  if (A)
    return A->getValue();
...
Tool *Generic_GCC::getTool(Action::ActionClass AC) const {
  ...
  case Action::PreprocessJobClass:
    if (!Preprocess)
      Preprocess.reset(new latino::driver::tools::gcc::Preprocessor(*this));
