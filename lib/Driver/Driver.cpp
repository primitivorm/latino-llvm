...
void Driver::CreateOffloadingDeviceToolChains(Compilation &C,
  ...
  if (IsCuda && IsHIP) {
    Diag(latino::diag::err_drv_mix_cuda_hip);
    return;
  }
  if (IsCuda) {
...
void Driver::generateCompilationDiagnostics(
  ...
  for (const char *TempFile : TempFiles) {
    Diag(latino::diag::note_drv_command_failed_diag_msg) << TempFile;
    if (Report)
  ...
  if (EC) {
    ...
    Diag(latino::diag::note_drv_command_failed_diag_msg) << Script;
  }
  ...
  for (const auto &A : C.getArgs().filtered(options::OPT_frewrite_map_file,
    ...
    Diag(latino::diag::note_drv_command_failed_diag_msg) << A->getValue();
...
int Driver::ExecuteCompilation(
  ...
  for (const auto &CmdPair : FailingCommands) {
    ...
    if (!FailingCommand->getCreator().hasGoodDiagnostics() || CommandRes != 1) {
      ...
      if (CommandRes < 0)
        Diag(latino::diag::err_drv_command_signalled)
            << FailingTool.getShortName();
      else
        Diag(latino::diag::err_drv_command_failed)
...
bool Driver::HandleImmediateArgs(const Compilation &C) {
  ...
  if (C.getArgs().hasArg(options::OPT_dumpversion)) {
    ...
    llvm::outs() << LATINO_VERSION_STRING << "\n";
    return false;
  }
...
void Driver::BuildUniversalActions(Compilation &C, const ToolChain &TC,
  ...
  for (Arg *A : Args) {
    if (A->getOption().matches(options::OPT_arch)) {
      ...
      if (Arch == llvm::Triple::UnknownArch) {
        Diag(latino::diag::err_drv_invalid_arch_name) << A->getAsString(Args);
        continue;
      }
  ...
  for (Action* Act : SingleActions) {
    ...
    if (Archs.size() > 1 && !types::canLipoType(Act->getType()))
      Diag(latino::diag::err_drv_invalid_output_with_multiple_archs)
...
bool Driver::DiagnoseInputExistence(const DerivedArgList &Args, StringRef Value,
  ...
  if (TypoCorrect) {
    ...
    if (getOpts().findNearest(Value, Nearest, IncludedFlagsBitmask,
      ...
      Diag(latino::diag::err_drv_no_such_file_with_suggestion)
          << Value << Nearest;
      return false;
    }
  }

  Diag(latino::diag::err_drv_no_such_file) << Value;
  return false;
}
...
void Driver::BuildInputs(const ToolChain &TC, DerivedArgList &Args,
  ...
  if (Arg *TCTP = Args.getLastArgNoClaim(options::OPT__SLASH_TC,
    ...
    for (Arg *A :
      ...
      if (Previous) {
        Diag(latino::diag::warn_drv_overriding_flag_option)
    ...
    if (ShowNote)
      Diag(latino::diag::note_drv_t_option_is_global);
  ...
  for (Arg *A : Args) {
    if (A->getOption().getKind() == Option::InputClass) {
      ...
      if (InputType == types::TY_Nothing) {
        ...
        if (memcmp(Value, "-", 2) == 0) {
          ...
          if (!Args.hasArgNoClaim(options::OPT_E) && !CCCIsCPP())
            Diag(IsCLMode() ? latino::diag::err_drv_unknown_stdin_type_clang_cl
                            : latino::diag::err_drv_unknown_stdin_type);
          Ty = types::TY_C;
        } else {
          ...
          if (CCCIsCXX()) {
            ...
            if (Ty != OldTy)
              Diag(latino::diag::warn_drv_treating_input_as_cxx)
      ...
      if (!InputType) {
        Diag(latino::diag::err_drv_unknown_language) << A->getValue();
    ...
    bool initialize() override {
      ...
      for (Arg *A : Args) {
        ...
        if (Arch == CudaArch::UNKNOWN) {
          C.getDriver().Diag(latino::diag::err_drv_cuda_bad_gpu_arch) << ArchStr;
          Error = true;
...
void Driver::handleArguments(Compilation &C, DerivedArgList &Args,
  ...
  if (YcArg && YuArg && strcmp(YcArg->getValue(), YuArg->getValue()) != 0) {
    Diag(latino::diag::warn_drv_ycyu_different_arg_clang_cl);
    Args.eraseArg(options::OPT__SLASH_Yc);
    Args.eraseArg(options::OPT__SLASH_Yu);
    YcArg = YuArg = nullptr;
  }
  if (YcArg && Inputs.size() > 1) {
    Diag(latino::diag::warn_drv_yc_multiple_inputs_clang_cl);
    Args.eraseArg(options::OPT__SLASH_Yc);
    YcArg = nullptr;
  }
  ...
  if (FinalPhase == phases::Link) {
    if (Args.hasArg(options::OPT_emit_llvm))
      Diag(latino::diag::err_drv_emit_llvm_link);
    if (IsCLMode() && LTOMode != LTOK_None &&
        !Args.getLastArgValue(options::OPT_fuse_ld_EQ).equals_lower("lld"))
      Diag(latino::diag::err_drv_lto_without_lld);
  }
  ...
  for (auto &I : Inputs) {
    ...
    if (InitialPhase > FinalPhase) {
      ...
      if (CCCIsCPP())
        Diag(latino::diag::warn_drv_input_file_unused_by_cpp)
      ...
      // more sense.
        ...
        Diag(latino::diag::warn_drv_preprocessed_input_file_unused)
            << InputArg->getAsString(Args) << !!FinalPhaseArg
            << (FinalPhaseArg ? FinalPhaseArg->getOption().getName() : "");
      else
        Diag(latino::diag::warn_drv_input_file_unused)
            << InputArg->getAsString(Args) << getPhaseName(InitialPhase)
            << !!FinalPhaseArg
            << (FinalPhaseArg ? FinalPhaseArg->getOption().getName() : "");
      continue;
...
void Driver::BuildActions(Compilation &C, DerivedArgList &Args,
  ...
  if (!SuppressMissingInputWarning && Inputs.empty()) {
    Diag(latino::diag::err_drv_no_input_files);
    return;
  }
  ...
  if (Arg *A = Args.getLastArg(options::OPT_Z_Joined))
    Diag(latino::diag::err_drv_use_of_Z_option) << A->getAsString(Args);
  ...
  if (Arg *A = Args.getLastArg(options::OPT__SLASH_Fo)) {
    ...
    if (Inputs.size() > 1 && !V.empty() &&
      ...
      Diag(latino::diag::err_drv_out_file_argument_with_multiple_sources)
          << A->getSpelling() << V;
  ...
  if (Arg *A = Args.getLastArg(options::OPT__SLASH_o)) {
    if (A->getValue()[0] == '\0') {
      ...
      Diag(latino::diag::err_drv_missing_argument) << A->getSpelling() << 1;
...
void Driver::BuildJobs(Compilation &C) const {
  ...
  if (FinalOutput) {
    ...
    for (const Action *A : C.getActions())
      if (A->getType() != types::TY_Nothing &&
          !(A->getKind() == Action::IfsMergeJobClass ||
            (A->getType() == latino::driver::types::TY_IFS_CPP &&
             A->getKind() == latino::driver::Action::CompileJobClass &&
             0 == NumIfsOutputs++) ||
    ...
    if (NumOutputs > 1) {
      Diag(latino::diag::err_drv_output_argument_with_multiple_files);
      FinalOutput = nullptr;
    }
  }
  ...
  for (Arg *A : C.getArgs()) {
    ...
    if (!A->isClaimed()) {
      ...
      if (!IsCLMode() || !A->getOption().matches(options::OPT_UNKNOWN))
        Diag(latino::diag::warn_drv_unused_argument)
...
const char *Driver::GetNamedOutputPath(Compilation &C, const JobAction &JA,
  ...
  if ((!AtTopLevel && !isSaveTempsEnabled() &&
    ...
    if (CCGenDiagnostics && A) {
      ...
      if (EC) {
        Diag(latino::diag::err_unable_to_make_temp) << EC.message();
        return "";
      }
    } else {
...
bool latino::driver::isOptimizationLevelFast(const ArgList &Args) {
  return Args.hasFlag(options::OPT_Ofast, options::OPT_O_Group, false);
}

bool latino::driver::willEmitRemarks(const ArgList &Args) {
