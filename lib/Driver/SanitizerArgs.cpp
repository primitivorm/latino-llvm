...
/// Sanitizers set.
static std::string toString(const latino::SanitizerSet &Sanitizers);
...
static void addDefaultBlacklists(const Driver &D, SanitizerMask Kinds,
  ...
  for (auto BL : Blacklists) {
    ...
    latino::SmallString<64> Path(D.ResourceDir);
    ...
    if (D.getVFS().exists(Path))
      ...
      // should fail.
      D.Diag(latino::diag::err_drv_no_such_file) << Path;
  }
  validateSpecialCaseListFormat(
      D, BlacklistFiles, latino::diag::err_drv_malformed_sanitizer_blacklist);
}
  ...
  for (const auto *Arg : Args) {
    ...
    if (Arg->getOption().matches(SCLOptionID)) {
      ...
      if (D.getVFS().exists(SCLPath)) {
        ...
        D.Diag(latino::diag::err_drv_no_such_file) << SCLPath;
      }
...
SanitizerArgs::SanitizerArgs(const ToolChain &TC,
  ...
  for (auto G : IncompatibleGroups) {
    ...
    if (Kinds & Group) {
      if (SanitizerMask Incompatible = Kinds & G.second) {
        D.Diag(latino::diag::err_drv_argument_not_allowed_with)
  ...
  parseSpecialCaseListArg(D, Args, UserBlacklistFiles,
                          ...
                          latino::diag::err_drv_malformed_sanitizer_blacklist);
    ...
    if (Arg *A =
      ...
      if (A->getOption().matches(options::OPT_fsanitize_memory_track_origins)) {
        ...
        if (S.getAsInteger(0, MsanTrackOrigins) || MsanTrackOrigins < 0 ||
            MsanTrackOrigins > 2) {
          D.Diag(latino::diag::err_drv_invalid_value) << A->getAsString(Args) << S;
        }
      }
    }
    MsanUseAfterDtor =
  ...
  if (MinimalRuntime) {
    ...
    if (NonTrappingCfi)
      D.Diag(latino::diag::err_drv_argument_only_allowed_with)
          << "fsanitize-minimal-runtime"
          << "fsanitize-trap=cfi";
  }
  ...
  if (CoverageFeatures & CoverageTraceBB)
    D.Diag(latino::diag::warn_drv_deprecated_arg)
  ...
  if (CoverageFeatures) {
    parseSpecialCaseListArg(
        ...
        latino::diag::err_drv_malformed_sanitizer_coverage_whitelist);
    parseSpecialCaseListArg(
        D, Args, CoverageBlocklistFiles,
        options::OPT_fsanitize_coverage_blocklist, OptSpecifier(),
        latino::diag::err_drv_malformed_sanitizer_coverage_blacklist);
  }
  ...
  if (AllAddedKinds & SanitizerKind::Address) {
    ...
    if (Arg *WindowsDebugRTArg =
      ...
      case options::OPT__SLASH_LDd:
        ...
        D.Diag(latino::diag::note_drv_address_sanitizer_debug_runtime);
      }
    }
    ...
    if (AllAddedKinds & DetectInvalidPointerPairs & ~AllRemove) {
      TC.getDriver().Diag(latino::diag::err_drv_argument_only_allowed_with)
  ...
  if (AllAddedKinds & SanitizerKind::HWAddress) {
    if (Arg *HwasanAbiArg =
      ...
      if (HwasanAbi != "platform" && HwasanAbi != "interceptor")
        D.Diag(latino::diag::err_drv_invalid_value)
...
SanitizerMask parseArgValues(const Driver &D, const llvm::opt::Arg *A,
  ...
  for (int i = 0, n = A->getNumValues(); i != n; ++i) {
    ...
    if (Kind)
      ...
      D.Diag(latino::diag::err_drv_unsupported_option_argument)
