...
void CodeGenModule::EmitModuleLinkOptions() {
  ...
  llvm::SetVector<latino::Module *> LinkModules;
  llvm::SmallPtrSet<latino::Module *, 16> Visited;
  SmallVector<latino::Module *, 16> Stack;
  ...
  // non-leaf modules.
  while (!Stack.empty()) {
    latino::Module *Mod = Stack.pop_back_val();
...
void CodeGenModule::EmitTopLevelDecl(Decl *D) {
  ...
  case Decl::Import: {
    ...
    while (!Stack.empty()) {
      ...
      // Visit the submodules of this module.
      for (latino::Module::submodule_iterator Sub = Mod->submodule_begin(),
