...
using namespace latino::CodeGen;
...
static llvm::DINode::DIFlags getAccessFlag(AccessSpecifier Access,
                                           const RecordDecl *RD) {
  AccessSpecifier Default = latino::AS_none;
  if (RD && RD->isClass())
    Default = latino::AS_private;
  else if (RD && (RD->isStruct() || RD->isUnion()))
    Default = latino::AS_public;

  if (Access == Default)
    return llvm::DINode::FlagZero;

  switch (Access) {
  case latino::AS_private:
    return llvm::DINode::FlagPrivate;
  case latino::AS_protected:
    return llvm::DINode::FlagProtected;
  case latino::AS_public:
...
llvm::DIModule *CGDebugInfo::getOrCreateModuleRef(ASTSourceDescriptor Mod,
  ...
  // latino::Module object, but it won't actually be built or imported; it will
  // be textual.
