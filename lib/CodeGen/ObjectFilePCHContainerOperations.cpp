...
#include "latino/CodeGen/ObjectFilePCHContainerOperations.h"
...
class PCHContainerGenerator : public ASTConsumer {
  ...
  struct DebugTypeVisitor : public RecursiveASTVisitor<DebugTypeVisitor> {
    latino::CodeGen::CGDebugInfo &DI;
    ASTContext &Ctx;
    DebugTypeVisitor(latino::CodeGen::CGDebugInfo &DI, ASTContext &Ctx)
        : DI(DI), Ctx(Ctx) {}
...
public:
  ...
  void HandleTranslationUnit(ASTContext &Ctx) override {
    ...
    if (Triple.isOSBinFormatMachO())
      ASTSym->setSection("__CLANG,__latinoAST");
    // COFF has an eight character length limit.
    else if (Triple.isOSBinFormatCOFF())
      ASTSym->setSection("latinoAST");
    else
      ASTSym->setSection("__latinoAST");

    LLVM_DEBUG({
      // Print the IR for the PCH container to the debug output.
      llvm::SmallString<0> Buffer;
      latino::EmitBackendOutput(
    ...
    latino::EmitBackendOutput(Diags, HeaderSearchOpts, CodeGenOpts, TargetOpts,
...
ObjectFilePCHContainerReader::ExtractPCH(llvm::MemoryBufferRef Buffer) const {
  ...
  if (OFOrErr) {
    ...
    for (auto &Section : OF->sections()) {
      ...
      if ((!IsCOFF && Name == "__latinoAST") || (IsCOFF && Name == "latinoAST")) {
