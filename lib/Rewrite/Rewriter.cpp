...
void RewriteBuffer::RemoveText(unsigned OrigOffset, unsigned Size,
  ...
  if (removeLineIfEmpty) {
    ...
    if (posI != end() && *posI == '\n') {
      ...
      // latino::Rewriter::RewriteOptions::RemoveLineIfEmpty.  A reproducer for
...
public:
  AtomicallyMovedFile(DiagnosticsEngine &Diagnostics, StringRef Filename,
    ...
    if (llvm::sys::fs::createUniqueFile(TempFilename, FD, TempFilename)) {
      AllWritten = false;
      Diagnostics.Report(latino::diag::err_unable_to_make_temp)
        << TempFilename;
    } else {
  ...
  ~AtomicallyMovedFile() {
    ...
    if (std::error_code ec = llvm::sys::fs::rename(TempFilename, Filename)) {
      AllWritten = false;
      Diagnostics.Report(latino::diag::err_unable_to_rename_temp)
