...
#include "latino/Parse/LoopHint.h"
...
/// PragmaOptimizeHandler - "\#pragma latino optimize on/off".
...
/// PragmaAttributeHandler - "\#pragma latino attribute ...".
struct PragmaAttributeHandler : public PragmaHandler {
  ...
  /// A pool of attributes that were parsed in \#pragma latino attribute.
...
void Parser::initializePragmaHandlers() {
  ...
  if (getLangOpts().CUDA) {
    ...
    PP.AddPragmaHandler("latino", CUDAForceHostDeviceHandler.get());
  }

  OptimizeHandler = std::make_unique<PragmaOptimizeHandler>(Actions);
  PP.AddPragmaHandler("latino", OptimizeHandler.get());

  LoopHintHandler = std::make_unique<PragmaLoopHintHandler>();
  PP.AddPragmaHandler("latino", LoopHintHandler.get());
  ...
  PP.AddPragmaHandler("latino", FPHandler.get());

  AttributePragmaHandler =
      std::make_unique<PragmaAttributeHandler>(AttrFactory);
  PP.AddPragmaHandler("latino", AttributePragmaHandler.get());

  MaxTokensHerePragmaHandler = std::make_unique<PragmaMaxTokensHereHandler>();
  PP.AddPragmaHandler("latino", MaxTokensHerePragmaHandler.get());

  MaxTokensTotalPragmaHandler = std::make_unique<PragmaMaxTokensTotalHandler>();
  PP.AddPragmaHandler("latino", MaxTokensTotalPragmaHandler.get());
}
...
void Parser::resetPragmaHandlers() {
  ...
  PP.RemovePragmaHandler("latino", PCSectionHandler.get());
  ...
  if (getLangOpts().CUDA) {
    PP.RemovePragmaHandler("latino", CUDAForceHostDeviceHandler.get());
  ...
  PP.RemovePragmaHandler("latino", OptimizeHandler.get());
  OptimizeHandler.reset();

  PP.RemovePragmaHandler("latino", LoopHintHandler.get());
  ...
  PP.RemovePragmaHandler("latino", FPHandler.get());
  FPHandler.reset();

  PP.RemovePragmaHandler("latino", AttributePragmaHandler.get());
  AttributePragmaHandler.reset();

  PP.RemovePragmaHandler("latino", MaxTokensHerePragmaHandler.get());
  MaxTokensHerePragmaHandler.reset();

  PP.RemovePragmaHandler("latino", MaxTokensTotalPragmaHandler.get());
...
static std::string PragmaLoopHintString(Token PragmaName, Token Option) {
  ...
  std::string ClangLoopStr = (llvm::Twine("latino loop ") + Str).str();
...
// #pragma latino section bss="abc" data="" rodata="def" text="" relro=""
void PragmaClangSectionHandler::HandlePragma(Preprocessor &PP,
  ...
  while (Tok.isNot(tok::eod)) {
    if (Tok.isNot(tok::identifier)) {
      PP.Diag(Tok.getLocation(), diag::err_pragma_expected_clang_section_name) << "latino section";
      return;
    }
    ...
    std::string SecName;
    if (!PP.LexStringLiteral(Tok, SecName, "pragma latino section", false))
      return;
...
void PragmaCommentHandler::HandlePragma(Preprocessor &PP,
  ...
  // issue a diagnostic.  Therefore neither does latino.
...
// #pragma latino optimize off
// #pragma latino optimize on
void PragmaOptimizeHandler::HandlePragma(Preprocessor &PP,
  ...
  if (Tok.is(tok::eod)) {
    PP.Diag(Tok.getLocation(), diag::err_pragma_missing_argument)
        << "latino optimize" << /*Expected=*/true << "'on' or 'off'";
    return;
  }
  if (Tok.isNot(tok::identifier)) {
...
/// Handle the \#pragma latino loop directive.
///  #pragma latino 'loop' loop-hints
///
///  loop-hints:
...
void PragmaLoopHintHandler::HandlePragma(Preprocessor &PP,
                                         PragmaIntroducer Introducer,
                                         Token &Tok) {
  // Incoming token is "loop" from "#pragma latino loop".
  Token PragmaName = Tok;
  SmallVector<Token, 1> TokenList;
...
/// isn't an intrinsic in latino and suggest to include intrin.h.
...
/// Handle the #pragma latino attribute directive.
///
/// The syntax is:
/// \code
///  #pragma latino attribute push (attribute, subject-set)
///  #pragma latino attribute push
///  #pragma latino attribute (attribute, subject-set)
///  #pragma latino attribute pop
/// \endcode
///
/// There are also 'namespace' variants of push and pop directives. The bare
/// '#pragma latino attribute (attribute, subject-set)' version doesn't require a
/// namespace, since it always applies attributes to the most recently pushed
/// group, regardless of namespace.
/// \code
///  #pragma latino attribute namespace.push (attribute, subject-set)
///  #pragma latino attribute namespace.push
///  #pragma latino attribute namespace.pop
/// \endcode
///
...
void PragmaAttributeHandler::HandlePragma(Preprocessor &PP,
  ...
  // Determine what action this pragma latino attribute represents.
  if (Tok.is(tok::l_paren)) {
    if (Info->Namespace) {
      PP.Diag(Tok.getLocation(),
...
// Handle '#pragma latino max_tokens 12345'.
void PragmaMaxTokensHereHandler::HandlePragma(Preprocessor &PP,
  ...
  if (Tok.is(tok::eod)) {
    PP.Diag(Tok.getLocation(), diag::err_pragma_missing_argument)
        << "latino max_tokens_here" << /*Expected=*/true << "integer";
    return;
  }
...
// Handle '#pragma latino max_tokens_total 12345'.
void PragmaMaxTokensTotalHandler::HandlePragma(Preprocessor &PP,
  ...
  if (Tok.is(tok::eod)) {
    PP.Diag(Tok.getLocation(), diag::err_pragma_missing_argument)
        << "latino max_tokens_total" << /*Expected=*/true << "integer";
    return;
  }
