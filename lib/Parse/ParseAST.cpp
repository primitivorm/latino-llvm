...
void PrettyStackTraceParserEntry::print(raw_ostream &OS) const {
  const latino::Token &Tok = P.getCurToken();
  if (Tok.is(latino::tok::eof)) {
  ...
  const latino::Preprocessor &PP = P.getPreprocessor();
...
/// held by Ctx.
///
void latino::ParseAST(latino::Preprocessor &PP, latino::ASTConsumer *Consumer, latino::ASTContext &Ctx, bool PrintStats,
              clang::TranslationUnitKind TUKind,
              latino::CodeCompleteConsumer *CompletionConsumer,
              bool SkipFunctionBodies) {
    std::unique_ptr<latino::Sema> S(
        new latino::Sema(PP, Ctx, *Consumer, TUKind, CompletionConsumer));
    
    // Recover resources if we crash before exiting this method.
    llvm::CrashRecoveryContextCleanupRegistrar<latino::Sema> CleanupSema(S.get());   

    ParseAST(*S.get(), PrintStats, SkipFunctionBodies);
}

void latino::ParseAST(latino::Sema &S, bool PrintStats, bool SkipFunctionBodies) {
    ...
    // std::swap(S.OldCollectStats, S.CollectStats);

  // Initialize the template instantiation observer chain.
  // FIXME: See note on "finalize" below.
  // initialize(S.TemplateInstCallbacks, S);

  latino::ASTConsumer *Consumer = &S.getASTConsumer();
