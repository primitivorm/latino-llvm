...
void FileManager::clearStatCaches() { StatCache.reset(); }
...
static const DirectoryEntry *getDirectoryFromFile(FileManager &FileMgr,
  ...
  return FileMgr.getDirectory(DirName, CacheFailed);
}

/// Add all ancestors of the given path (pointing to either a file or
/// a directory) as virtual directories.
void FileManager::addAncestorAsVirtualDirs(StringRef Path) {
  ...
  addAncestorAsVirtualDirs(DirName);
}
...
const FileEntry *FileManager::getVirtualFile(StringRef Filename, off_t Size,
  ...
  addAncestorAsVirtualDirs(Filename);
...
bool FileManager::getNoncachedStatValue(StringRef Path, vfs::Status &Result) {
  SmallString<128> FilePath(Path);
  FixupRelativePath(FilePath);
...
void FileManager::modifyFileEntry(FileEntry *File, off_t Size,
                                  time_t ModificationTime) {
  File->Size = Size;
