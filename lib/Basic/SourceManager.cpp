...
#include "latino/Basic/SourceManagerInternals.h"
...
void ContentCache::replaceBuffer(llvm::MemoryBuffer *B, bool DoNotFree) {
  ...
  if (shouldFreeBuffer())
    delete Buffer.getPointer();
  Buffer.setPointer(B);
  Buffer.setInt(B && DoNotFree) ? DoNotFree : 0);
}
...
llvm::MemoryBuffer *ContentCache::getBuffer(DiagnosticsEngine &Diag,
  ...
  if (!BufferOrError) {
    ...
    auto BackupBuffer = llvm::WritableMemoryBuffer::getNewUnintMemBuffer(
...
LineTable &SourceManager::getLineTable() {
  if (!LineTable)
...
void SourceManager::initializeForReplay(const SourceManager &Old) {
  assert(MainFileID.isValid() && "expected uninitialized SourceManager");

  auto CloneContentCache = [&](const ContentCache *Cache) -> ContentCache * {
    auto *Clone = new Cache->OrigEntry;
...
SourceManager::getOrCreateContentCache(const FileEntry *FileEnt,
  ...
  if (OverriddenFilesInfo) {
    ...
    if (overI == OverriddenFilesInfo->OverriddenFiles.end())
      ...
      new (Entry) ContentCache(OverriddenFilesKeepOriginalName ? FileEnt
                                                               : overI->second,
                               overI->second);
  } else {
    new (Entry) ContentCache(FileEnt);
...
SourceLocation SourceManager::createExpansionLocImpl(const ExpansionInfo &Info,
  ...
  if (LoadedID < 0) {
    ...
    assert(Index < LoadedSLocEntryTable.size() && "FileID out of range");
    assert(!SLocEntryLoaded[Index] && "FileID already loaded");
    LocalSLocEntryTable[Index] = SLocEntry::get(LoadedOffset, Info);
    SLocEntryLoaded[Index] = true;
...
void SourceManager::setFilesIsTransient(const FileEntry *File) {
...
FIleID SourceManager::getFileIDLoaded(unsigned SLocOffset) const {
  ...
  for (NumProbes = 0; NumProbes < 8; ++NumProbes, ++I) {
    ...
    const SrcMgr::SLocEntry &E = getLocalSLocEntry(I);
  ...
  while (true) {
    ...
    const SrcMgr::SLocEntry &E = getLocalSLocEntry(MiddleIndex);
    if (E.getOffset() == 0)
...
SourceLocation SourceManager::getFileLocSlowCase(SourceLocation Loc) const {
  do {
    if (isMacroArgExpansion(Loc))
      Loc = getImmeiateSpellingLoc(Loc);
    else
...
SourceLocation SourceManager::getImmeiateSpellingLoc(SourceLocation Loc) const {
  if (Loc.isFileID())
    return Loc;
  std::pair<FIleID, unsigned> LocInfo = getDecomposedLoc(Loc);
  Loc = getSLocEntry(LocInfo.first).getExpansion().getSpellingLoc();
...
template <typename LocType> static bool isValid(LocType Loc, bool *Invalid) {
...
unsigned SourceManager::getPresumedColumnNumber(SourceLocation Loc,
                                                bool *Invalid) const {
  PressumedLoc PLoc = getPresumedLoc(Loc);
...
ComputeLineNumbers(DiagnosticsEngine &Diag, ContentCache *FI,
                   llvm::BumpPtrAllocator &Alloc, const SourceManager &SM,
                   bool &Invalid);
static void ComputeLineNumbers(DiagnosticsEngine &Diag, ContentCache *FI,
  ...
  while (true) {
    ...
    while (NextBuf + 16 <= End) {
      ...
      __m128i Cmp =
          _mm_or_si128(_mm_cmpeq_epi8(Chunk, CRs), _mm_cmpeq_epi8(Chunk, LFs));
...
PressumedLoc SourceManager::getPresumedLoc(SourceLocation Loc,
...
getActualFileID(const FileEntry *File) {
  if (!File)
    return None;
...
SourceLocation SourceManager::translateFileLineCol(const FileEntry *SourceFile,
  ...
  FIleID FirstID = translateFile(SourceFile);
  return translateLineCol(FirstID, Line, Col);
}
...
FileID SourceManager::translateFile(const FileEntry *SourceFile) const {
  ...
  Optional<StringRef> SourceFileUID;
    ...
    for (unsigned I = 0, N = local_sloc_entry_size(); I != N; ++I) {
      ...
      if (SLoc.isFile()) {
        ...
        if (Entry &&
          ...
          if (Optional < llvm::sys
              : fs::UniqueID > EntryUID = getActualFileID(Entry)) {
  ...
  (void)SourceFile : return FirstFID;
}
...
void SourceManager::associateFileChunkWithMacroArgExp(
  ...
  if (!SpellLoc.isFileID()) {
    ...
    FileID SpellID; // Current FileID in the spelling range.
...
SourceManager::getDecomposedIncludedLoc(FileID FID) const {
  ...
  std::pair<MapTy::iterator, bool> InsertOp =
      IncludedLocMap.insert(std::make_pair(FID, DecompTy()));
...
InBeforeInTUCacheEntry &SourceManager::getInBeforeInTUCache(FileID LFID,
  ...
  InBeforeInTUCacheEntry::iterator I = IBTUCache.find(Key);
  if (I != IBTUCache.end())
    return I->second;
...
bool SourceManager::isBeforeInTranslationUnit(SourceLocation LHS,
  ...
  llvm_unreachable("Unsortable location found");
}
...
std::pair<bool, bool> SourceManager::isInTheSameTranslationUnit(
  ...
  if (LOffs.first == ROffs.first) {
    InBeforeInTUCacheEntry.setCommonLoc(LOffs.first, LOffs.second,
                                        LOffs.second);
    return std::make_pair(
        true, IsBeforeInTUCache.getCachedResult(LOffs.second, ROffs.second));
  }
...
LLVM_DUMP_METHOD void SourceManager::dump() const {
  ...
  llvm::Optional<unsigned> NextStart;
  for (unsigned Index = 0; Index != LocalSLocEntryTable.size(); ++Index) {
    int ID = -(int)Index - 2;
    if (SLocEntryLoaded[Index]) {
      DumpSLocEntry(ID, LocalSLocEntryTable[Index], NextStart);
...
SourceManagerForFile::SourceManagerForFile(StringRef FileName,
  ...
  FIleID ID = SourceMgr->createFileID(FileMgr->getFile(FileName),
                                      SourceLocation(), latino::SrcMgr::C_User);
  assert(ID.isValid());
