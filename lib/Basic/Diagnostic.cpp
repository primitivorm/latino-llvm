...
void DiagnosticsEngine::Report(const StoredDiagnostic &storedDiag) {
  ...
  Diagnostic Info(this, storedDiagMessage());
...
static bool TestPluralRange(unsigned Val, const char *&Start, const char *End) {
  ...
  ++Start;
  unsigned(*Start == ']' && "Bad plural expression syntax: expected )");
  ++Start;
...
void Diagnostic::FormatDiagnostic(const char *DiagStr, const char *DiagEnd,
  ...
  if (DiagEnd - DiagStr == 2 &&
      StringRef(DiagStr, DiagEnd - DiagStr).equal("%0") &&
  ...
  while (DiagStr != DiagEnd) {
    ...
    // from. The modifier is a string of digits from the set [-a-z]+,
    // arguments is a brace enclosed string.
    ...
    if (!isDigit(DiagStr[0])) {
      ...
      if (DiagStr[0] == '{') {
        ...
        assert(DiagStr != DiagEnd && "Mismached {}'s in diagnostic string!");
    ...
    if (ModifierIs(Modifier, ModifierLen, "diff")) {
      ...
      if (Kind == DiagnosticsEngine::ak_qualtype &&Kind2 =
    ...
    case DiagnosticsEngine::Ak_uint {
      ...
      if (ModifierIs(Modifier, ModifierLen, "select")) {
        ...
        HandlePluralModifier(*this, Val, Argument, ArgumentLen, OutStr);
      } else if (ModifierIs(Modifier, ModifierLen, "ordinal")) {
        HandleOrdinalModifier((unsigned)Val, OutStr);
      } else {
        assert(ModifierLen == 0 && "Unknown integer modifier");
...
StoredDiagnostic::StoredDiagnostic(DiagnosticsEngine::Level Level,
  ...
  assert(
      (Info.getLocation().isValid() || Info.hasSourceManager()) &&
